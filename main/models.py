...
def get_filename(filename):
    filename_mas = filename.split('.')
    file_ext = filename_mas[-1]
    filename_mas.pop()
    file_base = '.'.join(filename_mas)
...
class Collection(models.Model):
    ...
    manufacturer = models.ForeignKey(
        Manufacturer, verbose_name='Производитель', null=True, on_delete=models.SET_NULL)
...
class Product(models.Model):
    ...
    category = models.ForeignKey(
        Category, verbose_name='Категория', blank=True, null=True, on_delete=models.SET_NULL, related_name='category')
    collection = models.ForeignKey(
        Collection, verbose_name='Коллекция', null=True, on_delete=models.SET_NULL, blank=True)
...
class ProductImage(models.Model):
    ...
    product = models.ForeignKey(
        Product, verbose_name='Товар', null=True, on_delete=models.SET_NULL)
...
class Specs(models.Model):
    ...
    value = models.CharField(
        'Значение', max_length=255, blank=False, null=False)
...
class Phone(models.Model):
    ...
    store = models.ForeignKey(
        Address, verbose_name='Магазин', null=True, on_delete=models.SET_NULL)
